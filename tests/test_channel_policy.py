...
from fs25_network_core.channel_policy import COMMAND_CHANNELS, UNRESTRICTED_MEMBER_COMMANDS, require_command_channel
...
class ChannelPolicyTests(unittest.TestCase):
    def test_each_command_accepts_its_designated_channel(self):
        ...
        for command, channel_key in COMMAND_CHANNELS.items():
            with self.subTest(command=command):
                ...
                if command not in UNRESTRICTED_MEMBER_COMMANDS:
    ...
    def test_member_self_service_is_not_tied_to_a_channel(self):
        for command in UNRESTRICTED_MEMBER_COMMANDS:
            with self.subTest(command=command):
                require_command_channel(command, 1, 1, 999, {})
                with self.assertRaises(ValueError):
                    require_command_channel(command, 2, 1, 999, {})
