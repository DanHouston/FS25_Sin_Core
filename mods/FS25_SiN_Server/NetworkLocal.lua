...
function FS25SiNServer:processPermissionCommands()
    ...
    while true do
        ...
        if command ~= nil and not fileExists(receiptPath) then
            local worldMatches = self.worldIdentityReady == true and self.worldId ~= nil
                and tostring(self:commandWorldId(command) or "") == tostring(self.worldId)
            if not worldMatches then
